...
## 限制取舍（2026-09-02 空纪反馈：限制太多已影响体验）

放宽/去掉（删除理由 = 软限制/体验成本高）：

- `baseDir` 沙箱 → 全盘可访问——软限制（cd .. 能逃逸）防不住恶意却锁住正常使用，纯体验负担；删后可见"活在电脑里"的真实价值
- 输出截断 2000 → 12000 字符（保防爆越界，去体验割裂）
- 工具轮上限 3 → 8（连续工具链不再被硬断）
- 历史截断 12 → 40 条（长对话不再"失忆"）

保留（硬护栏，体验成本≈0）：15s 超时、工具失败不抛、轮次排队、非 2xx 抛错、注册表防重名、密钥不进 git。
