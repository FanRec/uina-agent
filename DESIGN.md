...
> 2026-09-02 现实修订（空纪指令：“先不考虑愿景，从一个最小 agent 出发，少即是多”）：
>
> - 验收 #2/#5（跨会话记忆、异步 Job）已撤回删除。最小切片的“记忆”= 会话续聊（--continue）。
> - 当前验收标准：流式回答 / 工具闭环回注 / 输出期间排队批量注入 / 跨重启会话恢复。
...
| 记忆整个模块（工具对 remember/recall/forget + 后端 port.ts + 自动 recall 注入） | 空纪定案“先从最小 agent 出发，少即是多”：长期记忆属于愿景#3，最小切片的记忆=会话续聊。工具对让模型凭空多两个决策维度；连自动 recall 注入一并退场，等真实需求（如向量检索/主动记住）再迭代回来 |
