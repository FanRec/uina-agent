...
import type { AgentMessage, ToolResultStatus } from "../src/core/types.js";
...
import { projectAgentHistory, projectModelHistory } from "../src/session/recovery.js";
...
describe("S1 durable session facts", () => {
	it("reopens a blocked call without execution, while a started unfinished call stays unknown", async () => {
		...
		const subject = new Subject(providerFor("probe"), broker, { onToken() {} }, { store, runtimeHooks: createRuntimeHooks(host) });
	...
	it("reopens domain custom messages in compaction tails without promoting custom entries", async () => {
		const path = await sessionPath();
		const { store } = await openJsonlSession(path);
		const tail: AgentMessage[] = [
			{ role: "user", content: "before" },
			{ role: "custom", customType: "observation", content: "observed", display: false, details: { origin: "fixture" } },
			{ role: "compactionSummary", summary: "earlier", content: "earlier summary", tokensBefore: 12 },
			{ role: "assistant", content: "after" },
		];
		await store.appendCustomEntry({ customType: "private", data: { secret: "not model content" } });
		await store.appendCompaction("summary", tail, 50);
		await store.close();
		const reopened = await openJsonlSession(path);
		await reopened.store.close();
	...
	it("commits queued steer and followUp once during a normal continuation", async () => {
		...
		let calls = 0;
		const subject = new Subject({ name: "fixture", async stream(_req, delta) {
			if (++calls === 1) { entered(); await gate; }
			delta({ kind: "text", text: "done" }); delta({ kind: "finish", reason: "stop" });
		} }, new ToolBroker(), { onToken() {} }, { store });
	...
	it("leaves input pending and reports a failed commit without automatic retry", async () => {
		...
		const errors: string[] = [];
		const subject = new Subject(scriptedProvider([{ match: () => true, produce: () => [{ kind: "text", text: "done" }] }]), new ToolBroker(), { onToken() {}, onError: error => errors.push(error) }, { store });
		const run = subject.pushInput("first");
	...
	it("resumes a recovered queue through the same input commit", async () => {
		...
		const subject = new Subject(scriptedProvider([{ match: () => true, produce: () => [{ kind: "text", text: "done" }] }]), new ToolBroker(), { onToken() {} }, { store: resumed.store });
...
describe("S1 tool outcome propagation", () => {
	it.each(["succeeded", "failed", "cancelled", "unknown", "not_started"] as ToolResultStatus[])("preserves %s through hooks, live UI and JSONL replay", async status => {
		...
		const subject = new Subject(providerFor("probe"), broker, {
			onToken() {},
			onTurnStart: (n, text) => tui.render({ type: "turn_start", n, text }),
			onToolStart: (name, args, callId) => tui.render({ type: "tool_start", name, args, callId }),
			onToolDone: (name, result, status, callId) => tui.render({ type: "tool_done", name, result, status, callId }),
	...
	it("records a real shell exit 7 as failed in the hook, event and message", async () => {
		...
		const subject = new Subject(providerFor(execCommand.def.function.name, { command: "exit 7" }), broker, { onToken() {} }, { store, runtimeHooks: createRuntimeHooks(host) });
